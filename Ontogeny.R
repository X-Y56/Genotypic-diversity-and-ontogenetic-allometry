...
fig.MeanHeight <- ggplot(MonthlyData, aes(x = Day, y = MeanHeight, color = Diversity)) +
  ...
  geom_line(data = daily_means, aes(x = Day, y = MeanHeight), linetype='dashed') + 
  ylab('Mean height (cm)') +
...
fig.Ramet <- ggplot(MonthlyData, aes(x = Day, y = Ramet, color = Diversity)) +
  ...
  geom_line(data = daily_means, aes(x = Day, y = Ramet), linetype='dashed') + 
  ylab('Ramet number') +
  xlab('Julian day of year') +
  scale_colour_manual(name = 'Diversity', values = SpeciesCol) +  
  theme_bw() +
...
fig.Inflorescence <- ggplot(MonthlyData, aes(x = Day, y = Inflorescence, color = Diversity)) +
  ...
  geom_line(data = daily_means, aes(x = Day, y = Inflorescence), linetype='dashed') + 
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##                                                                            ~~
##                SEM: SOIL LEGACY, TIMING AND ALLOMETRY                  ----
##                                                                            ~~
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
names(PlotData)
names(PeakRamet)
names(PeakInflorescence)
names(PeakMeanHeight)


## Peak ramet timing and magnitude
SEM.PeakRamet <- PeakRamet %>%
  dplyr::select(Plot, Composition, Diversity, Day, Ramet) %>%
  dplyr::rename(
    PeakRametDay    = Day,
    PeakRametNumber = Ramet
  )


## Peak inflorescence timing and magnitude
SEM.PeakInflorescence <- PeakInflorescence %>%
  dplyr::select(Plot, Composition, Diversity, Day, Inflorescence) %>%
  dplyr::rename(
    PeakInflorescenceDay    = Day,
    PeakInflorescenceNumber = Inflorescence
  )


## Peak height timing and magnitude
SEM.PeakHeight <- PeakMeanHeight %>%
  dplyr::select(Plot, Composition, Diversity, Day, MeanHeight) %>%
  dplyr::rename(
    PeakHeightDay = Day,
    PeakHeight    = MeanHeight
  )


## Combine peak timing and magnitude data
SEM.PeakData <- SEM.PeakRamet %>%
  left_join(SEM.PeakInflorescence,
            by = c("Plot", "Composition", "Diversity")) %>%
  left_join(SEM.PeakHeight,
            by = c("Plot", "Composition", "Diversity"))


## Combine SMA allometry, soil data and peak data
SEMData <- PlotData %>%
  left_join(SEM.PeakData,
            by = c("Plot", "Composition", "Diversity")) %>%
  mutate(
    Composition = as.factor(Composition),
    LogPeakRametNumber = log(PeakRametNumber),
    LogPeakInflorescenceNumber = log(PeakInflorescenceNumber + 1),
    LogPeakHeight = log(PeakHeight)
  )
...
SEM.variables <- c(
  "Plot", "Composition", "Diversity",
  "AP", "MBC", "F_shannon",
  "PeakRametDay", "PeakRametNumber", "PeakInflorescenceNumber",
  "slope.Ramet", "elevation.Ramet"
)

SEM.variables[!SEM.variables %in% names(SEMData)]
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
## This model tests:
## 1) whether developmental timing is associated with ramet-height allometry;
## 2) whether nutrient-related and microbial-related soil variables contribute
##    to timing, allometry and plant performance.

sem1 <- psem(
  ## soil legacy pathways
  lmer(AP ~ Diversity +   (1 | Composition),data = SEMData),
  lmer(MBC ~ Diversity + (1 | Composition), data = SEMData),
  lmer(F_shannon ~ Diversity + (1 | Composition), data = SEMData),
  lmer(NO3 ~ Diversity + (1 | Composition), data = SEMData),
  ## developmental timing
  lmer(PeakRametDay ~Diversity+ AP + MBC + NO3 + F_shannon + (1 | Composition),  data = SEMData),
  lmer(slope.Ramet ~ PeakRametDay + AP + NO3  + F_shannon + (1 | Composition), data = SEMData),
  lmer(elevation.Ramet ~ PeakRametDay + AP + NO3  + F_shannon + (1 | Composition),data = SEMData),
  ## clonal performance
  lmer(PeakRametNumber ~ PeakRametDay + slope.Ramet + elevation.Ramet + (1 | Composition), data = SEMData),
  ## reproductive output
  lmer(PeakInflorescenceNumber ~ PeakRametNumber + (1 | Composition), data = SEMData),
  ## covariance between ramet-height allometric parameters
  slope.Ramet %~~% elevation.Ramet,data = SEMData
)
...
sem1.1 <- psem(
  ...
  lmer(elevation.Ramet ~ PeakRametDay + AP + NO3  + F_shannon + (1 | Composition),data = SEMData),
  ## clonal performance
  lmer(PeakRametNumber ~ Diversity+PeakRametDay + slope.Ramet + elevation.Ramet + (1 | Composition), data = SEMData),#new added Diversity
  ## reproductive output
  lmer(PeakInflorescenceNumber ~ PeakRametNumber + MBC+PeakRametDay +(1 | Composition), data = SEMData),#added  MBC+PeakRametDay
  ## covariance between ramet-height allometric parameters
  slope.Ramet %~~% elevation.Ramet,data = SEMData,
  MBC %~~% AP,data = SEMData#new added
)
...
#remove unsignificant path of sem1.1:
...
sem1.2 <- psem(
  ...
  #lmer(NO3 ~ Diversity + (1 | Composition), data = SEMData),
  ## developmental timing
  lmer(PeakRametDay ~Diversity+ (1 | Composition),  data = SEMData),
  lmer(slope.Ramet ~  AP + NO3  + F_shannon + (1 | Composition), data = SEMData),
  lmer(elevation.Ramet ~  AP + NO3  + F_shannon + (1 | Composition),data = SEMData),
  ## clonal performance
  lmer(PeakRametNumber ~ Diversity + slope.Ramet + elevation.Ramet + (1 | Composition), data = SEMData),#new added Diversity
  ## reproductive output
  lmer(PeakInflorescenceNumber ~ PeakRametNumber + MBC+PeakRametDay +(1 | Composition), data = SEMData),#added  MBC+PeakRametDay
  ## covariance between ramet-height allometric parameters
  slope.Ramet %~~% elevation.Ramet,data = SEMData,
...
SEM.coefs <- coefs(sem1.2)
SEM.R2    <- rsquared(sem1.2)



#write.xlsx(SEM.coefs, file = "Result/Table_SEM_path_coefficients.xlsx", rowNames = FALSE)

#write.xlsx(SEM.R2,file = "Result/Table_SEM_R2.xlsx",  rowNames = FALSE)
